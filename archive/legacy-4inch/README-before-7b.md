...
**Current hardware target:** the user uses a Waveshare ESP32-S3-Touch-LCD-7B
(7-inch, 1024×600). See the [7B hardware reference](docs/HARDWARE_7B_REFERENCE.md)
from `bambu-p1s-led`, branch `feat/display-ui`. The firmware and original hardware
instructions below still target the 4-inch board; the 7B port is not implemented yet.

See the [7B migration requirements](docs/WAVESHARE_7B_MIGRATION_REQUIREMENTS.md)
for the change checklist, implementation order, and acceptance criteria.

See the [current UI approval draft](docs/UI_7B_REVIEW.md); feature selection checklist is no longer required.

A native Windows [7B UI simulator](simulator/7b/README.md) is available at
1024×600 with demo telemetry. Its layout preview does not yet implement the ESP32 hardware port.
...
### Lisp Settings Reference

See [Lisp settings over CAN](docs/LISP_SETTINGS_REFERENCE.md) for the reviewed
[huygithcm/esp32p4-android-auto](https://github.com/huygithcm/esp32p4-android-auto)
implementation: dynamic Lisp controls, CAN actions/state, persistence, and REPL.
This is an implementation reference; two-way Lisp settings are not yet integrated here.
